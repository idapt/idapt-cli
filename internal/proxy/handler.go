...
// NewGetHandler creates an HTTP handler for GET /api/proxy.
// Returns the current proxy config (which ports are TLS-exposed).
...
// NewPostHandler creates an HTTP handler for POST /api/proxy.
// Updates the proxy config: validates, saves to disk, reconciles TLS listeners.
