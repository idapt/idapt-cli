...
// Proxy is a reverse proxy that forwards requests to local backend services.
type Proxy struct {
	defaultPort int
	transport   http.RoundTripper
}

// New creates a new reverse proxy targeting the given default backend port.
...
// ServeHTTP proxies the request to the appropriate backend port.
...
// resolvePort determines which backend port to proxy to based on the request.
// Dynamic listeners inject their port into the context — proxy to localhost:same_port.
// The main :443 listener proxies to the configured default backend port.
...
// Minimal bad gateway HTML (>512 bytes to avoid browser/CDN interception)
