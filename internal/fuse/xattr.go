...
// writableXattrs are the attributes that can be SET via the API.
...
// Listxattr returns all supported xattr names.
func (n *IdaptNode) Listxattr(ctx context.Context, dest []byte) (uint32, syscall.Errno) {
	// Build null-terminated list of xattr names
	var size uint32
...
// Setxattr sets an extended attribute value (only writable ones).
...
// Removexattr is not supported — xattrs are server-derived.
