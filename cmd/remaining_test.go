		...
		"GET /api/shared-with-me": func(w http.ResponseWriter, r *http.Request) {
			jsonResponse(w, 200, map[string]interface{}{
				"shares": []map[string]interface{}{
					{"id": "sh1", "resourceType": "kb", "resourceName": "Wiki", "access": "read", "sharedBy": "bob"},
				},
			})
		},
	})
...
func TestShareCreate(t *testing.T) {
	h := mockHandler(map[string]func(w http.ResponseWriter, r *http.Request){
		"POST /api/kb/kb-1/shares": func(w http.ResponseWriter, r *http.Request) {
			var body map[string]interface{}
			json.NewDecoder(r.Body).Decode(&body)
	...
	stdout, _, err := runCmd(t, h, "share", "create", "kb", "kb-1", "--email", "bob@example.com", "--access", "write")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestShareCreateMissingEmail(t *testing.T) {
	h := mockHandler(map[string]func(w http.ResponseWriter, r *http.Request){})
	_, _, err := runCmd(t, h, "share", "create", "kb", "kb-1")
	if err == nil {
...
func TestShareDelete(t *testing.T) {
	h := mockHandler(map[string]func(w http.ResponseWriter, r *http.Request){
		"DELETE /api/kb/kb-1/shares/sh1": func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(204)
		},
	})
	stdout, _, err := runCmd(t, h, "share", "delete", "kb", "kb-1", "sh1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
