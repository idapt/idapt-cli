...
Use --max-files and --max-size to enforce limits (e.g., for Lambda execution).`,
...
// syncLimits holds configurable sync limits (0 = unlimited).
...
// versionMap tracks fileId → version for incremental sync.
// Stored as .idapt-versions.json in the sync directory.
...
// syncDown downloads server files to local directory. Incremental: skips files
// whose version matches the cached version from the previous sync.
func syncDown(ctx context.Context, client *ifuse.FuseAPIClient, projectID, localPath string, exclusion *isync.ExclusionEngine, limits syncLimits, out io.Writer) error {
	...
	// Load previous version map for incremental sync
	prevVersions := loadVersionMap(localPath)
	newVersions := make(versionMap)

	// Collect all server files first (for limit checking + batch version compare)
	var allFiles []fileWithPath
	if err := collectServerFiles(ctx, client, projectID, "", "", exclusion, &allFiles); err != nil {
		return err
	}

	// Check limits before downloading anything (fail fast)
	...
	// Batch version check — find which files changed since last sync
	...
	// Download files — skip unchanged ones
	...
	for _, f := range allFiles {
		...
		// Incremental: skip if version matches previous sync
		serverV := serverVersions[f.entry.ID]
		prevV := prevVersions[f.entry.ID]
		if serverV > 0 && prevV == serverV {
			// File exists locally and hasn't changed on server
			if _, err := os.Stat(localFilePath); err == nil {
				newVersions[f.entry.ID] = serverV
				skipped++
				continue
			}
			// File deleted locally — re-download
		}
	...
	// Save version map for next incremental sync
...
// syncUp uploads local files to server.
...
func syncUpDir(ctx context.Context, client *ifuse.FuseAPIClient, projectID, parentID, localDir, relativePath string, exclusion *isync.ExclusionEngine, out io.Writer) error {
	...
	for _, entry := range entries {
		...
		// Skip hidden files, version map, and known non-syncable patterns
...
// fileWithPath pairs a FileEntry with its relative path in the tree.
type fileWithPath struct {
	entry   ifuse.FileEntry
	relPath string
}

// collectServerFiles recursively collects all files from the server.
