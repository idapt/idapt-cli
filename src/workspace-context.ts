...
export async function resolveWorkspaceRef(
  ...
  if (ref.toLowerCase() === "default") {
    const marked = rows.find(isDefault);
    if (marked) return toLookup(marked as Record<string, unknown>);
  }
...
export async function runWorkspaceContext(
  ...
  if (!ref) {
    ...
    for (const w of rows) {
      const key = pick(w, "owner_slug", "ownerSlug") ?? "(no team)";
    ...
    const defaultIdx = rows.findIndex(isDefault);
    if (defaultIdx >= 0) {
      const defaultOwner = groups.find(
        (g) =>
          g.key ===
          (pick(rows[defaultIdx], "owner_slug", "ownerSlug") ?? "(no team)"),
      );
      if (defaultOwner && groups[0] !== defaultOwner) {
        groups.splice(groups.indexOf(defaultOwner), 1);
        groups.unshift(defaultOwner);
      }
    }
    const lines: string[] = [
    ...
    for (const group of groups) {
      if (showHeaders) lines.push(`  ${group.key}/`);
      for (const w of group.rows) {
        const current = resourceIdOf(w) === creds.defaultWorkspaceId;
        lines.push(
          `${current ? "*" : " "} ${qualifiedSlug(w) ?? resourceIdOf(w)}  ${displayName(w)}`,
        );
      }
    }
    lines.push("");
