...
function wallHints(code: UsageWallCode): string[] {
  switch (code) {
    case "credits_required":
      return [
        "This costs credits and your spendable balance cannot cover it.",
        "Add credits, or bring your own keys (BYOK): https://idapt.app/pricing",
      ];
    case "credits_unlock_required":
      return [
        "This op is outside the promotional band (the welcome credit only covers ops up to $1).",
        "Add credits (your first top-up unlocks every model) or use BYOK: https://idapt.app/pricing",
      ];
    case "credits_exhausted":
      return ["Not enough credits. Add credits to continue."];
...
export function requestIdOf(error: IdaptError | undefined): string | null {
  const body = error?.body as
