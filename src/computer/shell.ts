...
function terminalErrorMessage(res: {
  status: number;
  json?: unknown;
  text?: string | undefined;
}): string {
  const structured = (res.json as { error?: { message?: string } } | undefined)
    ?.error?.message;
  if (structured) return structured;
  const text = res.text?.trim() ?? "";
  const looksLikeMarkup = /^\s*<(?:!doctype|html|\?xml)/i.test(text);
  if (text && !looksLikeMarkup && text.length <= 300) return text;
  return `could not start a terminal (HTTP ${res.status})`;
}
...
export async function runShell(
  ...
  const res = await transport.request({
    method: "GET",
    path: `/api/v1/computers/${encodeURIComponent(computer)}/pty-token`,
    query: Object.fromEntries(query),
  });
  if (res.status >= 400 || !res.json) {
    io.err(`✗ ${terminalErrorMessage(res)}\n`);
