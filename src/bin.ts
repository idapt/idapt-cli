...
import { clientHelpFor } from "./client-help";
import {
  COMPLETION_SHELLS,
...
import { type GlobalFlags, parseGlobalFlags, validateVerbFlags } from "./flags";
...
function prefixCliError(message: string): string {
  return message.startsWith("idapt: ") ? message : `idapt: ${message}`;
}

async function resolveCredentialOrExplain(
  io: RunIo,
  args: Parameters<typeof resolveCredential>[0],
): Promise<{ credential: ResolvedCredential | null; refused: boolean }> {
  try {
    return { credential: await resolveCredential(args), refused: false };
  } catch (err) {
    if (err instanceof CredentialOriginError) {
      io.stderr(`${err.message}\n`);
      return { credential: null, refused: true };
    }
    return { credential: null, refused: false };
  }
}
...
export async function run(io: RunIo): Promise<number> {
  const { globals, rest, restFlagNames, errors } = parseGlobalFlags(io.argv);
  if (errors.length > 0) {
    io.stderr(`${errors.map(prefixCliError).join("\n")}\n`);
  ...
  if (globals.version || rest[0] === "version") {
    io.stdout(
      globals.output === "json"
        ? `${JSON.stringify(versionFacts(baseUrl), null, 2)}\n`
        : `${versionBlock(baseUrl)}\n`,
    );
    return EXIT_OK;
  }
  ...
  if (globals.help) {
    const usage = clientHelpFor(rest);
    if (usage) {
      io.stdout(`${usage}\n`);
      return EXIT_OK;
    }
  ...
  if (cmd !== "login" && !(cmd === "auth" && rest[1] === "login")) {
    const contextError = unknownContextError({
      flag: globals.context,
      env: io.env,
    });
    if (contextError) {
      io.stderr(`${contextError}\n`);
      return EXIT_VALIDATION;
    }
  }
  ...
  if (cmd === "whoami")
    return runStatus(ctx, globals.context, globals.output === "json");
  if (cmd === "auth") {
    const sub = rest[1];
    if (sub === "login") {
  ...
  if (cmd === "open") {
    ...
    return runOpen(globals.help ? ["--help"] : rest.slice(1), {
      print: io.stdout,
      err: io.stderr,
      baseUrl,
      isTty: io.isTty,
  ...
  const flagErrors = validateVerbFlags(
    restFlagNames.filter((f) => !RESERVED_INPUT_FLAGS.has(f)),

    [
      ...declared,
      ...declared.flatMap((field) =>
        (ARG_ALIASES[field] ?? []).filter((a) => !declared.includes(a)),
      ),
      ...spec.pathParams,
      "json",
    ],
  ...
  if (flagErrors.length > 0) {
    io.stderr(`${flagErrors.map(prefixCliError).join("\n")}\n`);
    return EXIT_VALIDATION;
  }

  const auth = await resolveCredentialOrExplain(io, {
  ...
  if (auth.refused) return EXIT_AUTH;
  ...
  let scopedWorkspaceLabel: string | undefined;

  const isAccountDefaultRef =
    scope === "scope" && workspaceRef?.toLowerCase() === "default";
  if (scope === "scope" && workspaceRef && !isAccountDefaultRef) {
    ...
    if (credential.isWorkspaceScoped) {
      ...
      scopedWorkspaceLabel = lookup.qualified ?? lookup.label;
    }
  }
  ...
  let restForCommand = rest;
  let fileFromPositional: string | undefined;
  if (spec.argLocation === "multipart" && !globals.file) {

    const parsed = parseInvocation(buildCommandFromArgv(rest));
    const bare = parsed.ok
      ? (resolveCommandForCli(parsed.invocation.pathTokens)?.rest ?? [])
      : [];

    fileFromPositional = bare[spec.pathParams.length];
    if (fileFromPositional) {
      const dropAt = rest.lastIndexOf(fileFromPositional);
      if (dropAt >= 0) {
        restForCommand = [...rest.slice(0, dropAt), ...rest.slice(dropAt + 1)];
      }
    }
  }

  const restWithJson = resolveSecretArgSource(
    resolveJsonArgSource(restForCommand, fileIo),
    fileIo,
  );
  ...
  if (
    ...
    io.stderr(`Working in ${scopedWorkspaceLabel}.\n`);
  }
  ...
  const localFile = globals.file ?? fileFromPositional;
  if (localFile) {
    try {
      const part = await openLocalFilePart(localFile);
      filePart = part.file;
  ...
  if ("thrown" in result) {
    ...
    return result.thrown instanceof TransportTimeoutError
      ? EXIT_TIMEOUT
      : EXIT_ERROR;
  }
  ...
  if (result.ok) {
    ...
    const rawPassthrough =
      spec.responseKind === "binary" && globals.output === undefined;
    if (rawPassthrough && typeof result.data === "string") {

      io.stdout(result.data);
    } else if (rawPassthrough && result.data instanceof Blob) {
      if (io.isTty) {

        io.stderr(
          `${result.data.size} bytes of binary. Redirect it to a file:\n` +
            `  idapt ${spec.command} ${docPath.slice(2).join(" ")} > out\n`,
        );
      } else {
        const bytes = new Uint8Array(await result.data.arrayBuffer());
        (io.stdoutBytes ?? ((b) => io.stdout(Buffer.from(b).toString())))(
          bytes,
        );
      }
    ...
    if (spec.responseKind === "created" && mode === "table" && io.isTty) {
      const id = createdIdOf(result.data);
      io.stderr(
        id ? `Created ${spec.resource} ${id}\n` : `Created ${spec.resource}.\n`,
      );
    }

    if (result.pageHint) io.stderr(`${result.pageHint}\n`);
    ...
    const pendingToken = pendingTokenOf(result.data);
    if (pendingToken) {
      io.stderr(
        `\nAccepted and still running.\n  Collect the result with: idapt ${spec.resource} messages ${pendingToken.chatId}\n`,
      );
    }
  } else {
    io.stderr(
        ...
        formatError(
          result.cause ?? new Error(result.error ?? result.rendered),
          {
            ...
            ...(spec.featureFlag
              ? {
                  gatedFeature: {
                    flag: spec.featureFlag,
                    enabled: (creds.enabledFeatures ?? []).includes(
                      spec.featureFlag,
                    ),
                  },
                }
  ...
  if (result.status === 0) {
    return result.failure === "packaging" ? EXIT_ERROR : EXIT_VALIDATION;
  }
  return exitCodeForStatus(result.status);
}

function createdIdOf(data: unknown): string | undefined {
  if (!data || typeof data !== "object") return undefined;
  const row = data as Record<string, unknown>;
  for (const key of ["id", "resource_id", "slug"]) {
    const value = row[key];
...
function pendingTokenOf(data: unknown): { chatId: string } | undefined {
  if (!data || typeof data !== "object") return undefined;
  const row = data as { status?: unknown; chat_id?: unknown };
  if (row.status !== "pending" || typeof row.chat_id !== "string") {
    return undefined;
  }
...
async function runAmbientContextSubverb(
  ...
  const { credential: cred, refused } = await resolveCredentialOrExplain(io, {
...
function versionFacts(baseUrl: string): Record<string, string> {
  return {
    version: VERSION,
    commit: COMMIT,
    built_at: BUILT_AT,
