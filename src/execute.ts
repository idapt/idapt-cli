...
import { parseInvocation, toSnakeKey } from "./parser";
import {
  ARG_ALIASES,
  mapArgsToV1,
  PATH_PARAM_ALIASES,
...
const ARG_MISMAP_HINTS: Record<string, string> = {
  compose_text:
    "Compose documents belong to `container compose-up` (or `container compose-set`), not create.",
};

function unknownArgsIn(
  spec: V1CommandSpec,
  ...
  return Object.keys(rest).filter((key) => {
    ...
    if (rest[key] instanceof Blob) return false;
    const snake = toSnakeKey(key);
    return !(snake in shape);
  });
}
...
async function runSpec(
  ...
  const unknownArgs = unknownArgsIn(spec, bound.rest);
  if (unknownArgs.length > 0) {
    const hints = unknownArgs
      .map((key) => {
        const hint = ARG_MISMAP_HINTS[toSnakeKey(key)];
        return hint ? `unknown argument \`${toSnakeKey(key)}\`: ${hint}` : null;
      })
      .filter((h): h is string => h !== null);
    return err(
      spec.command,
      0,
      `invalid arguments for \`${spec.command}\`: unknown argument${unknownArgs.length > 1 ? "s" : ""} ${unknownArgs
        .map((key) => `\`${toSnakeKey(key)}\``)
        .join(
          ", ",
        )}. ${hints.join(" ")} Run \`idapt help ${spec.command}\` for the contract.`,
    );
  }
