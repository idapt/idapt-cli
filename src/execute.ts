...
function requestDeclaresWorkspaceId(
  request: V1CommandSpec["request"],
): boolean {
  const shape = (request as { shape?: Record<string, unknown> } | null)?.shape;
...
export async function execute(
  ...
  const mergedArgs =
    ...
    requestDeclaresWorkspaceId(spec.request)
