...
import { globalFlagNames } from "./flags";
import { type CachedId, cachedIds } from "./id-cache";
...
export function completeWords(words: readonly string[]): string[] {
  ...
  if (prior.length === 0) {
    const candidates = [
      ...new Set([...listResources("cli"), ...EXTRA_TOP_LEVEL]),
    ];
  ...
  if (prior.length === 1) {
    ...
    const verbs = commandsForResource(resource, "cli").map((c) =>
  ...
  const flags = [...globalFlagNames(), "--json"];
  if (toComplete.startsWith("-")) return filterSorted(flags, toComplete);
  const resource = prior[0] as string;
  const ids = idsForCompletion(resource).map((c) => c.id);
  return filterSorted([...ids, ...flags], toComplete);
}

let idsForCompletion: (resource: string) => readonly CachedId[] = cachedIds;

export function setIdSource(
  source: (resource: string) => readonly CachedId[],
): void {
  idsForCompletion = source;
}
