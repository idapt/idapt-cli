...
import { CredentialOriginError, resolveCredential } from "./resolve";
...
function issuedAt(baseUrl: string): { apiUrl?: string } {
  try {
    return { apiUrl: new URL(baseUrl).origin };
  } catch {
    return {};
  }
}
...
export async function runLogin(
  ...
  if (opts.apiKeyStdin) {
    ...
    saveActiveCredentials(
      { apiKey: key, ...issuedAt(ctx.baseUrl) },
      ctxOpts(opts),
    );
  ...
  try {
    if (useDevice) {
      ...
      saveActiveCredentials(
        { apiKey: sessionToken, ...issuedAt(ctx.baseUrl) },
        ctxOpts(opts),
      );
...
export async function runStatus(
  ...
  let resolved: Awaited<ReturnType<typeof resolveCredential>> = null;
  try {
