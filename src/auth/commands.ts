...
export async function runLogin(
  ...
  if (opts.apiKeyStdin) {
    ...
    saveActiveCredentials({ apiKey: key });
  ...
  try {
    if (useDevice) {
      ...
      saveActiveCredentials({ apiKey: sessionToken });
...
export async function runStatus(ctx: AuthCtx): Promise<number> {
  ...
  const lines: string[] = [
    ...
    `Kind:      ${resolved.kind}`,
    `Workspace: ${creds.defaultWorkspaceSlug ?? creds.defaultWorkspaceId ?? "personal (default)"}`,
