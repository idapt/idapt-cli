...
function callbackPage(opts: {
  ...
  const accent = opts.tone === "ok" ? "#3086ff" : "#d4443c";
  return `<!doctype html>
<html lang="en"><head>
<meta charset="utf-8">
...
<style>
  ...
  .mark { width: 56px; height: 56px; margin: 0 auto 1.5rem; display: block; }
  h1 { font-size: 1.35rem; font-weight: 650; margin: 0 0 0.5rem; letter-spacing: -0.01em; }
  p { margin: 0; color: #5c5f66; line-height: 1.5; font-size: 0.95rem; }
  .rule { width: 40px; height: 3px; border-radius: 2px; background: ${accent}; margin: 1.5rem auto 0; }
  ...
  <main class="card">
    <svg class="mark" viewBox="0 0 100 100" aria-hidden="true" fill="none">
      <circle cx="50" cy="22" r="11" fill="#3086ff"/>
      <circle cx="22" cy="72" r="11" fill="#3086ff"/>
      <circle cx="78" cy="72" r="11" fill="#3086ff"/>
      <path d="M50 33 L28 62 M50 33 L72 62" stroke="#3086ff" stroke-width="7" stroke-linecap="round"/>
    </svg>
    <h1>${opts.heading}</h1>
    <p>${opts.body}</p>
    <div class="rule"></div>
...
const CALLBACK_OK = callbackPage({
  title: "Signed in to idapt",
  heading: "You're signed in",
  body: "Logged in to your terminal. You can close this tab.",
  tone: "ok",
});

function callbackError(reason: string): string {
  return callbackPage({
    title: "Sign-in failed",
    heading: "Sign-in failed",
    body: `${reason} Return to your terminal and run idapt login again.`,
    tone: "error",
  });
}
...
export function loginAuthCode(io: FlowIo): Promise<OAuthTokens> {
  ...
  return new Promise<OAuthTokens>((resolve, reject) => {
    ...
    const server = createServer((req, res) => {
      ...
      if (err) {
        res
          .writeHead(400, CALLBACK_HEADERS)
          .end(
            callbackError(
              err === "access_denied"
      ...
      if (q.get("state") !== state) {
        res
          .writeHead(400, CALLBACK_HEADERS)
          .end(callbackError("This response did not match the request."));
      ...
      if (!code) {
        res
          .writeHead(400, CALLBACK_HEADERS)
          .end(callbackError("The response carried no authorization code."));
        finish(() =>
          reject(
      ...
      res.writeHead(200, CALLBACK_HEADERS).end(CALLBACK_OK);
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      io.out("Approve the sign-in in your browser to continue.");
      io.out("Waiting for sign-in. Press Ctrl-C to cancel.");
    });
  });
}
...
export function pollInterval(stated: number | undefined): number {
...
export async function loginDevice(io: FlowIo): Promise<string> {
  ...
  let interval = pollInterval(code.interval);
  const deadline =
