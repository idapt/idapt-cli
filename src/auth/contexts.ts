...
export function unknownContextError(opts: {
  flag?: string | undefined;
  env?: Record<string, string | undefined> | undefined;
}): string | null {
  const requested = opts.flag || opts.env?.IDAPT_CONTEXT;
  if (!requested) return null;
  const store = loadContexts();
  if (store.contexts[requested]) return null;

  const known = Object.keys(store.contexts).sort();
  const source = opts.flag ? "--context" : "IDAPT_CONTEXT";
  const suffix = known.length
    ? `Your accounts: ${known.join(", ")}.`
    : "You have no stored accounts yet.";
  return (
    `idapt: no account named "${requested}" (${source}). ${suffix}\n` +
    `  Create it with \`idapt login --context ${requested}\`.`
  );
}
        ...
        `${JSON.stringify(
          names.sort().map((name) => {
            ...
            return {
              ...
              api_url: ctx.apiUrl ?? null,
              workspace: ctx.defaultWorkspaceSlug ?? null,
            };
          }),
          null,
          2,
