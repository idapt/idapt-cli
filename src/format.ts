...
function humanCell(value: unknown, field: string): string {
  ...
  if (TIME_FIELD.test(field) && value) return relativeTime(value);
  return cell(value);
}
...
function renderTable(
  ...
  if (rows.length === 0) {
    ...
    const label = options.emptyLabel ?? `${spec.resource}s`;
    return paint(`No ${label} found.`, "dim", color);
  }

  const cols = resolveColumns(data, spec, options);
  const headers = cols.map((c) => c.header);
  const body = rows.map((row) =>
    cols.map((c) => humanCell(readPath(row, c.field), c.field)),
  );

  const widths = fitWidths(headers, body, options.width ?? terminalWidth());
