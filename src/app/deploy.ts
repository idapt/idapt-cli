...
const DEPLOY_FLAGS = ["app", "workspace", "name"];
...
export function parseDeployArgs(args: readonly string[]): DeployOptions & {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === "--app") app = args[++i];
    ...
    else if (a === "--workspace" || a === "--workspace-id" || a === "-w")
      workspace = args[++i];
      ...
      const suggestion = closestMatch(flag, DEPLOY_FLAGS);
      errors.push(
        suggestion
          ? `unknown flag ${a} for \`app deploy\`. Did you mean --${suggestion}?`
          : `unknown flag ${a} for \`app deploy\`. Accepts: ${DEPLOY_FLAGS.map((f) => `--${f}`).join(", ")}.`,
      );
    } else if (dir === undefined) dir = a;
  }
  return { dir: dir ?? "", app, workspace, name, errors };
}
